...
tags:
  ...
  audience: [mod-user]
  subject: [ltk-manager, patcher, creator-workshop]
---
...
2. Under **Installation path**, select **Auto-detect**.
3. If LTK Manager still does not find League, select the folder button in the field and choose
   the folder that contains `Game/League of Legends.exe`.
...
LTK Manager is currently Windows-only. The cslol patcher stopped working on macOS, so there is no working mod manager for macOS at this time. A new macOS patcher is being developed alongside a native macOS LTK Manager release, targeted for Q2/Summer 2026.

### Linux

LTK Manager does not yet support Linux. A Linux release is planned but has no release date, and it will only include the Creator Workshop and the features it depends on - the patcher will never be supported on Linux, so you will be able to make mods but not play with them. Follow [LeagueToolkit on GitHub](https://github.com/LeagueToolkit) for updates.
