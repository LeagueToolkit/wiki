...
tags:
  type: guide
  audience: [mod-user]
  subject: [ltk-manager, load-order, layers]
---
...
- **`.modpkg`** - Modern mod package format
- **`.fantome`** - Format from the cslol era, still the most common one on mod sites
...
Mod enable/disable state is per-profile - see [Profiles](/mod-management/profiles/) for details.
