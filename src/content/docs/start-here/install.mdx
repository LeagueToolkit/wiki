...
import PlatformBadges from '../../../components/PlatformBadges.svelte';
...
<PlatformBadges
  client:visible
  platforms={[
    { name: 'Windows 10+', platform: 'windows', status: 'supported', note: '64-bit' },
    { name: 'macOS', platform: 'macos', status: 'coming-soon', note: 'Q2/Summer 2026' },
    { name: 'Linux', platform: 'linux', status: 'coming-soon', note: 'Planned (workshop only)' },
  ]}
/>

:::caution[macOS and Linux users]
LTK Manager is currently Windows-only. The cslol patcher stopped working on macOS, and a new macOS patcher is in development alongside a native macOS LTK Manager release, targeted for Q2/Summer 2026. Linux support is planned but has no release date yet, and it covers only the Creator Workshop and the features it depends on - the patcher will never be supported on Linux, so you can make mods there but not play with them.
:::
...
- [Workshop Overview](/making-mods/workshop/) - Start creating your own mods
