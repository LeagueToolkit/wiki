...
tags:
  ...
  subject: [layers, creator-workshop, load-order]
---
...
String overrides are layer-specific, so different layers can provide different text. See [String Overrides](/making-mods/string-overrides/) for the full editing guide.
...
Each layer contains `.wad.client` folders that assign raw files to their target WAD. Place your mod files inside the appropriate `.wad.client` directory using the game's file path structure.
